...
MainWindow.Size = UDim2.new(0, 200, 0, 350)
MainWindow.Position = UDim2.new(0.5, -100, 0.5, -175)  -- 屏幕中心
...
ms.Color = Color3.fromRGB(50, 100, 255)
ms.Thickness = 1.5
ms.Transparency = 0.3
ms.Parent = MainWindow
...
local function StartFloatAnimation()
    ...
    floatConnection = RunService.RenderStepped:Connect(function(dt)
        ...
        local offset = math.sin(tick() * 1.2) * 3
        MainWindow.Position = UDim2.new(0.5, -100, 0.5, -175 + offset)
    end)
end
StartFloatAnimation()
...
RestoreBtn.Position = UDim2.new(0.5, -20, 0.5, -20)  -- 中心
...
local function CreateTab(name, pos)
    ...
    tab.Size = UDim2.new(0, 39, 0, 22)
    tab.Position = UDim2.new(0, pos, 0, 1)
...
local FuncTab   = CreateTab("功能", 43)
local FunTab    = CreateTab("娱乐", 84)
local CombatTab = CreateTab("战斗", 125)
local PlayerTab = CreateTab("玩家", 166)
ESPTab.BackgroundColor3 = Color3.fromRGB(50, 100, 255)
ESPTab.TextColor3 = Color3.fromRGB(255, 255, 255)

local function CreateContent()
    local c = Instance.new("Frame")
    c.Size = UDim2.new(1, -12, 1, -58)
    c.Position = UDim2.new(0, 6, 0, 54)
...
local FunContent = CreateContent()
...
local function CreateButton(parent, x, y, w, h, text, active)
    ...
    st.Color = Color3.fromRGB(50, 100, 255)
    st.Thickness = 1
    st.Transparency = 0.5
    st.Parent = btn
    return btn
...
local ESPBoxBtn   = CreateButton(ESPContent, 0,  34, 90,  26, "方框: 关", false)
local ESPNameBtn  = CreateButton(ESPContent, 96, 34, 90,  26, "名字: 关", false)
local ESPHealthBtn= CreateButton(ESPContent, 0,  64, 90,  26, "血量: 关", false)
local ESPDistBtn  = CreateButton(ESPContent, 96, 64, 90,  26, "距离: 关", false)
local ESPTraceBtn = CreateButton(ESPContent, 0,  94, 186, 26, "射线: 关", false)

-- ==================== 功能标签页 ====================
local FlyBtn = CreateButton(FuncContent, 0, 2, 90, 26, "飞行", false)
local SpinBtn = CreateButton(FuncContent, 96, 2, 90, 26, "自转", false)
local JumpBtn = CreateButton(FuncContent, 0, 32, 90, 26, "无限跳", false)
local NoClipBtn = CreateButton(FuncContent, 96, 32, 90, 26, "穿墙", false)
local SpeedBtn = CreateButton(FuncContent, 0, 62, 90, 26, "加速1x", false)
local StopBtn = CreateButton(FuncContent, 96, 62, 90, 26, "停止全部", false)
StopBtn.BackgroundColor3 = Color3.fromRGB(60, 20, 20)

-- ==================== 娱乐标签页 ====================
local CircleBtn   = CreateButton(FunContent, 0,  2,  90, 26, "绕圈", false)
local HandsUpBtn  = CreateButton(FunContent, 96, 2,  90, 26, "举手", false)
local SitBtn      = CreateButton(FunContent, 0,  32, 60, 26, "摔倒", false)
local FreezeBtn   = CreateButton(FunContent, 64, 32, 60, 26, "冻结", false)
local FlingBtn    = CreateButton(FunContent, 128,32, 58, 26, "弹飞", false)
local SpinTargetBtn = CreateButton(FunContent, 0,  62, 60, 26, "转圈", false)
local FlipBtn     = CreateButton(FunContent, 64, 62, 60, 26, "倒立", false)
local UnTrollBtn  = CreateButton(FunContent, 128,62, 58, 26, "恢复", false)

local CircleTargetLabel = Instance.new("TextLabel")
CircleTargetLabel.Size = UDim2.new(1, 0, 0, 14)
CircleTargetLabel.Position = UDim2.new(0, 0, 0, 95)
CircleTargetLabel.BackgroundTransparency = 1
CircleTargetLabel.Text = "目标: 无"
CircleTargetLabel.TextColor3 = Color3.fromRGB(150, 180, 220)
CircleTargetLabel.TextSize = 9
CircleTargetLabel.Font = Enum.Font.GothamMedium
CircleTargetLabel.Parent = FunContent

-- ==================== 战斗标签页 ====================
local AimbotToggle = CreateButton(CombatContent, 0, 2, 90, 26, "自瞄", false)
local AimbotVisBtn = CreateButton(CombatContent, 96, 2, 90, 26, "可视检查", false)
local HitboxToggle = CreateButton(CombatContent, 0, 32, 90, 26, "范围伤害", false)
local HitboxUpBtn = CreateButton(CombatContent, 96, 32, 44, 26, "+", false)
local HitboxDownBtn = CreateButton(CombatContent, 142, 32, 44, 26, "-", false)
local HitboxSizeLabel = Instance.new("TextLabel")
HitboxSizeLabel.Size = UDim2.new(1, 0, 0, 16)
HitboxSizeLabel.Position = UDim2.new(0, 0, 0, 62)
...
PlayerModeLabel.Size = UDim2.new(1, 0, 0, 16)
...
PlayerModeLabel.TextSize = 10
PlayerModeLabel.Font = Enum.Font.GothamBold
PlayerModeLabel.Parent = PlayerContent
local ModeSwitchBtn = CreateButton(PlayerContent, 0, 20, 186, 22, "切换为绕圈模式", false)
local PlayerList = Instance.new("ScrollingFrame")
PlayerList.Size = UDim2.new(1, 0, 0, 200)
PlayerList.Position = UDim2.new(0, 0, 0, 46)
...
local function AnimateWindow(show)
    ...
    if show then
        ...
        -- 淡入 + 缩放动画
        MainWindow.BackgroundTransparency = 0.8
        MainWindow.Size = UDim2.new(0, 180, 0, 315)
        MainWindow.Position = UDim2.new(0.5, -90, 0.5, -157)
        
        local t1 = TweenService:Create(MainWindow, tweenInfo, {
        ...
        -- 淡出 + 缩放动画
        local t1 = TweenService:Create(MainWindow, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        ...
        t1.Completed:Connect(function()
            ...
            local t2 = TweenService:Create(RestoreBtn, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
...
local function StartFlying()
    ...
    Components.FlyConnection = RunService.Heartbeat:Connect(function()
        ...
        local fw = cam.CFrame.LookVector * Vector3.new(1,0,1)
        local rt = cam.CFrame.RightVector * Vector3.new(1,0,1)
    ...
    Components.FlyKeyBegan = UserInputService.InputBegan:Connect(function(i,g)
        if g then return end
        if i.KeyCode == Enum.KeyCode.W then md += Vector3.new(0,0,-1)
        elseif i.KeyCode == Enum.KeyCode.S then md += Vector3.new(0,0,1)
        elseif i.KeyCode == Enum.KeyCode.A then md += Vector3.new(-1,0,0)
        elseif i.KeyCode == Enum.KeyCode.D then md += Vector3.new(1,0,0)
        elseif i.KeyCode == Enum.KeyCode.Space then ud = 1
        elseif i.KeyCode == Enum.KeyCode.LeftControl then ud = -1 end
    end)
    ...
    Components.FlyKeyEnded = UserInputService.InputEnded:Connect(function(i)
        ...
        elseif i.KeyCode == Enum.KeyCode.A then md -= Vector3.new(-1,0,0)
        elseif i.KeyCode == Enum.KeyCode.D then md -= Vector3.new(1,0,0)
        elseif i.KeyCode == Enum.KeyCode.Space then ud = 0
        elseif i.KeyCode == Enum.KeyCode.LeftControl then ud = 0 end
    end)
end
...
local function StartSpinning()
    ...
    av.AngularVelocity = Vector3.new(0,70,0)
    av.Parent = root
    Components.SpinAngularVelocity = av
end
...
local function StartCircling(target)
    ...
    Components.CircleConnection = RunService.Heartbeat:Connect(function()
        ...
        local tc = Components.CircleTarget.Character
        if not tc then StopCircling(); return end
        local tr = tc:FindFirstChild("HumanoidRootPart")
        if not tr or not root or not root.Parent then StopCircling(); return end
...
local function ToggleNoClip()
    ...
    if State.NoClip then
        ...
        Components.NoClipConnection = char.DescendantAdded:Connect(function(p)
...
local function CycleSpeed()
    ...
    for i,v in ipairs(speeds) do
        if v == State.Speed then idx = i; break end
    end
    idx = idx % #speeds + 1
...
local function TrollHandsUp(target)
    ...
    if torso and torso:FindFirstChild("Right Shoulder") then
        torso["Right Shoulder"].CurrentAngle = math.rad(180)
        torso["Left Shoulder"].CurrentAngle = math.rad(180)
    end
end
...
local function TrollSpin(target)
    ...
    task.spawn(function()
        while bg and bg.Parent do
            bg.CFrame = bg.CFrame * CFrame.Angles(0, math.rad(10), 0)
            task.wait()
        end
    end)
    table.insert(Components.TrollConnections, bg)
...
local function ClearTroll()
    for _, obj in pairs(Components.TrollConnections) do
...
local function GetClosestPlayer()
    ...
    for _, pl in pairs(Players:GetPlayers()) do
        ...
        if State.AimbotVisible then
            local _, os = Camera:WorldToViewportPoint(hd.Position)
            ...
            if ry then
                local hc = ry.Instance:FindFirstAncestorOfClass("Model")
                if hc ~= pl.Character then continue end
            end
        end
...
local function RestoreHitbox(pts)
    for _, d in pairs(pts) do
        if d.part and d.part.Parent then d.part.Size = d.oldSize end
    end
end
    ...
    if State.Hitbox then
        for _, pl in pairs(Players:GetPlayers()) do
            if pl ~= LocalPlayer and pl.Character then
                Components.HitboxParts[pl] = ExpandHitbox(pl.Character)
            end
        end
        Components.HitboxConnection = Players.PlayerAdded:Connect(function(pl)
            pl.CharacterAdded:Connect(function(ch)
                task.wait(0.5)
                if State.Hitbox then Components.HitboxParts[pl] = ExpandHitbox(ch) end
            end)
        end)
    else
...
local function UpdateESP()
    for player, d in pairs(ESPDrawings) do
        ...
        if not ch or not ch:FindFirstChild("HumanoidRootPart") or not ch:FindFirstChild("Humanoid") then
            for _, v in pairs(d) do v.Visible = false end
            continue
        end
        if not root then continue end
        local tr = ch.HumanoidRootPart
        local th = ch.Humanoid
        local pos, os = Camera:WorldToViewportPoint(tr.Position)
        if not os then
            for _, v in pairs(d) do v.Visible = false end
            continue
        end
        ...
        local bs = Vector2.new(40*scale, 60*scale)
        local bp = Vector2.new(pos.X-bs.X/2, pos.Y-bs.Y/2)
        ...
        if State.ESP and State.ESPHealth then
            ...
            local hp = math.clamp(h/mh,0,1)
            d.HealthBg.Visible = true
            d.HealthBg.Size = Vector2.new(3, bs.Y)
            d.HealthBg.Position = Vector2.new(bp.X-6, bp.Y)
            d.HealthBar.Visible = true
            d.HealthBar.Size = Vector2.new(3, bs.Y*hp)
            d.HealthBar.Position = Vector2.new(bp.X-6, bp.Y+bs.Y*(1-hp))
        ...
        if State.ESP and State.ESPDistance then
            ...
            d.Distance.Position = Vector2.new(pos.X, bp.Y+bs.Y+4)
        ...
        if State.ESP and State.ESPTracers then
            ...
            d.Tracer.To = Vector2.new(pos.X, bp.Y+bs.Y)
            d.Tracer.Color = Color3.fromRGB(255,255,255)
...
local function ToggleESP()
    ...
    if State.ESP then
        ...
        local pa = Players.PlayerAdded:Connect(function(pl)
            if pl ~= LocalPlayer then task.wait(1); CreateESP(pl) end
        end)
        table.insert(Components.ESPConnections, pa)
        local pr = Players.PlayerRemoving:Connect(function(pl)
            if ESPDrawings[pl] then
                for _, v in pairs(ESPDrawings[pl]) do v:Remove() end
                ESPDrawings[pl] = nil
            end
        end)
        ...
        ESPDrawings = {}
        for _, c in pairs(Components.ESPConnections) do c:Disconnect() end
...
local function ToggleESPName()
    ...
    ESPNameBtn.BackgroundColor3 = State.ESPName and Color3.fromRGB(30,80,40) or Color3.fromRGB(30,40,60)
    ESPNameBtn.Text = State.ESPName and "名字: 开" or "名字: 关"
end
...
local function RefreshPlayerList()
    ...
    for _, pl in pairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer then
            local n = #playerButtons + 1
            local rw = math.floor((n-1)/2)
            local cl = (n-1)%2
            local btn = Instance.new("TextButton")
            btn.Size = UDim2.new(0, 88, 0, 24)
            btn.Position = UDim2.new(0, cl*92, 0, rw*28)
            ...
            btn.MouseButton1Click:Connect(function()
                if playerListMode == "teleport" then
                    if TeleportToPlayer(pl) then
                        btn.BackgroundColor3 = Color3.fromRGB(30,80,40)
                        task.wait(0.3)
                        btn.BackgroundColor3 = Color3.fromRGB(40,40,60)
                    end
                else
                    Components.SelectedCircleTarget = pl
                    CircleTargetLabel.Text = "目标: "..pl.Name
    ...
    PlayerList.CanvasSize = UDim2.new(0, 0, 0, math.max(math.ceil(#playerButtons/2)*28, 40))
end

ModeSwitchBtn.MouseButton1Click:Connect(function()
    playerListMode = (playerListMode == "teleport") and "circle" or "teleport"
    PlayerModeLabel.Text = "模式: "..(playerListMode == "teleport" and "传送" or "绕圈")
    ModeSwitchBtn.Text = (playerListMode == "teleport") and "切换为绕圈模式" or "切换为传送模式"
end)
...
CircleBtn.MouseButton1Click:Connect(function()
    ...
    elseif Components.SelectedCircleTarget then
        ...
        CircleTargetLabel.Text = "目标: "..Components.SelectedCircleTarget.Name
    end
end)
...
HitboxUpBtn.MouseButton1Click:Connect(function()
    ...
    if State.Hitbox then ToggleHitbox(); ToggleHitbox() end
end)
