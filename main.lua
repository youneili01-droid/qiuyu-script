...
-- ==================== ESP标签页 - 5个按钮 ====================
local ESPToggle  = CreateButton(ESPContent, 0, 2,  186, 28, "ESP总开关: 关", false)  -- 按钮1
local ESPBoxBtn  = CreateButton(ESPContent, 0, 34, 90,  28, "方框: 关", false)      -- 按钮2
local ESPHealthBtn = CreateButton(ESPContent, 96,34, 90,  28, "血量: 关", false)      -- 按钮3
local ESPDistBtn  = CreateButton(ESPContent, 0, 66, 90,  28, "距离: 关", false)      -- 按钮4
local ESPTraceBtn = CreateButton(ESPContent, 96,66, 90,  28, "射线: 关", false)      -- 按钮5
...
-- ==================== 功能函数（飞行/自转/绕圈/无限跳/穿墙/加速/停止/传送/自瞄/范围） ====================
...
local function CreateESP(player)
    local drawings = {}
    local box = Drawing.new("Square"); box.Visible = false; box.Color = Color3.fromRGB(255, 255, 255); box.Thickness = 1.5; box.Filled = false; box.Transparency = 0.7; drawings.Box = box
    local nameTag = Drawing.new("Text"); nameTag.Visible = false; nameTag.Color = Color3.fromRGB(255, 255, 255); nameTag.Size = 13; nameTag.Center = true; nameTag.Outline = true; nameTag.Font = 3; drawings.Name = nameTag
    local healthBg = Drawing.new("Square"); healthBg.Visible = false; healthBg.Color = Color3.fromRGB(30, 30, 30); healthBg.Thickness = 1; healthBg.Filled = true; drawings.HealthBg = healthBg
    local healthBar = Drawing.new("Square"); healthBar.Visible = false; healthBar.Color = Color3.fromRGB(0, 255, 0); healthBar.Thickness = 1; healthBar.Filled = true; drawings.HealthBar = healthBar
    local distance = Drawing.new("Text"); distance.Visible = false; distance.Color = Color3.fromRGB(255, 255, 255); distance.Size = 12; distance.Center = true; distance.Outline = true; distance.Font = 3; drawings.Distance = distance
    local tracer = Drawing.new("Line"); tracer.Visible = false; tracer.Color = Color3.fromRGB(255, 255, 255); tracer.Thickness = 1; tracer.Transparency = 0.5; drawings.Tracer = tracer
    ESPDrawings[player] = drawings
end

local function UpdateESP()
    for player, drawings in pairs(ESPDrawings) do
        if not player or not player.Parent then for _, d in pairs(drawings) do d:Remove() end; ESPDrawings[player] = nil; continue end
        local character = player.Character
        if not character or not character:FindFirstChild("HumanoidRootPart") or not character:FindFirstChild("Humanoid") then for _, d in pairs(drawings) do d.Visible = false end; continue end
        ...
        if not onScreen then for _, d in pairs(drawings) do d.Visible = false end; continue end
        ...
        if State.ESPBox then
            ...
            drawings.Box.Color = (player.TeamColor and player.TeamColor.Color) or Color3.fromRGB(255, 255, 255)
        else drawings.Box.Visible = false end
        
        drawings.Name.Visible = State.ESP; drawings.Name.Position = Vector2.new(position.X, boxPos.Y - 18); drawings.Name.Text = player.Name
        ...
        if State.ESPHealth then
            ...
            drawings.HealthBar.Color = hp > 0.6 and Color3.fromRGB(0,255,0) or (hp > 0.3 and Color3.fromRGB(255,255,0) or Color3.fromRGB(255,0,0))
...
RunService.RenderStepped:Connect(function()
    if State.ESP then UpdateESP() else for _, d in pairs(ESPDrawings) do for _, v in pairs(d) do v.Visible = false end end end
end)
...
local function ToggleESP()
    ...
    ESPToggle.Text = State.ESP and "ESP总开关: 开" or "ESP总开关: 关"
    if State.ESP then
        ...
        local pr = Players.PlayerRemoving:Connect(function(pl) if ESPDrawings[pl] then for _, d in pairs(ESPDrawings[pl]) do d:Remove() end; ESPDrawings[pl] = nil end end)
...
local function RefreshPlayerList()
    ...
    for _, pl in pairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer then
            local n = #playerButtons + 1; local rw = math.floor((n-1)/2); local cl = (n-1)%2
            local btn = Instance.new("TextButton")
